...
<p>${attr1}</p><%--page 영역에서 꺼냄--%>
<p>${attr2}</p><%--page 영역에서 꺼냄--%>
...
<%--session 영역에 추가--%>
<c:set var="attr4" value="hello attributr4" scope="session"/>
<%--session 영역에서 꺼냄--%>
<p>attr4 : ${attr4}</p>

<%--application 영역에 추가--%>
<c:set var="attr5" value="hello attributr5" scope="application"/>
<%--application 영역에서 꺼냄--%>
<p>${attr5}</p>
...
<%--session 영역 : 하나의 브라우저에서 오는 여러 requst가 공유하는 영역--%>
<c:set var="car" value="테슬라" scope="session"/>
<p>${car}</p>

<%--application 영억 : 어플리케이션 내의 모든 곳에서 사용 가능한 영억--%>
